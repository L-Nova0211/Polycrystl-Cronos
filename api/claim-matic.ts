...
const {
  VERCEL_URL, 
  REACT_APP_CRO_FAUCET_PAYOUT_AMOUNT, 
  REACT_APP_CRO_FAUCET_MAX_BALANCE, 
  CRO_FAUCET_PRIVATE_KEY,
  CRO_FAUCET_GAS
} = process.env;
...
export default async (request: VercelRequest, response: VercelResponse) => {
  try {
    console.log('---new request:claim-cro---')
    ...
    const payoutAmount = parseFloat(REACT_APP_CRO_FAUCET_PAYOUT_AMOUNT);
    const maxBalance = parseFloat(REACT_APP_CRO_FAUCET_MAX_BALANCE);
    web3NoAccount.eth.accounts.wallet.add(CRO_FAUCET_PRIVATE_KEY)
    ...
    if (isAddress(sanitizedAddress)) {
      ...
      if (accountBalance >= maxBalance) {
        ...
        return response.status(429).json({
          err: `You already have ${accountBalance} CRO!`
        });
      }
