...
const user = process.env.REACT_APP_CRONOS_NODE_USERNAME
const password = process.env.REACT_APP_CRONOS_NODE_PASSWORD

const httpProvider = new Web3.providers.HttpProvider(RPC_URL, {
  headers: user &&
    password && [{ name: 'Authorization', value: `Basic ${Buffer.from(`${user}:${password}`).toString('base64')}` }],
  timeout: 120000,
