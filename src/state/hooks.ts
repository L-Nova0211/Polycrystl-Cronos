...
export const useGetPriceFromSymbol = (address: string, decimals = 18, isLP = false): number => {
  const { slowRefresh } = useRefresh()
  const priceGetterContract = usePriceGetter()
  const [symbolPrice, setSymbolPrice] = useState<number>()

  useEffect(() => {
    async function fetchSymbolPrice() {
      let price: any
      if (isLP) price = await priceGetterContract.methods.getLPPrice(address, decimals).call()
      else price = await priceGetterContract.methods.getPrice(address, decimals).call()
      setSymbolPrice(price)
    }
    fetchSymbolPrice()
  }, [address, decimals, isLP, priceGetterContract.methods, slowRefresh])
