...
import { Box, Button, Flex, HelpIcon, LinkExternal, MetamaskIcon, Skeleton, Text, useTooltip } from '@crystals/uikit'
...
const ActionPanel: React.FC<ActionPanelProps> = ({ account, pool, userDataLoaded, expanded, breakpoints }) => {
  const { /* sousId,  */ stakingToken, earningToken, totalStaked, stakingLimit, isAutoVault, contractAddress } = pool
  const { t } = useTranslation()
  const { currentBlock } = useBlock()
  const { isXs, isSm, isMd } = breakpoints
  const showSubtitle = (isXs || isSm) && false /* sousId === 0 */
  ...
  const performanceFeeAsDecimal = 0 /* performanceFee && performanceFee / 100 */
  ...
  return (
    <StyledActionPanel expanded={expanded}>
      <InfoSection>
        ...
        <Flex mb="8px" justifyContent={['flex-end', 'flex-end', 'flex-start']}>
          <LinkExternal href={`${BASE_BSC_SCAN_URL}/address/${poolContractAddress}`} bold={false}>
            {t('View Contract')}
          </LinkExternal>
        </Flex>
